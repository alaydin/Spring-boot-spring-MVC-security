...
CREATE TABLE `members` (
  ...
  `pw` char(60) NOT NULL,
  `active` tinyint NOT NULL DEFAULT 1, -- Set the default value to 1
  PRIMARY KEY (`user_id`)
...
CREATE TABLE roles (
  ...
  CONSTRAINT authorities5_ibfk_1 FOREIGN KEY (user_id) REFERENCES members (user_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
